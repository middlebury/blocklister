...
try {
	...
	// Add a regular expressions that will match IP ranges that we should never add to the blacklist.
	// This will prevent accidentally blacklisting certain clients that are trusted.
	$blacklist->addWhitelistPattern('/^140\.233\./');
	$blacklist->addWhitelistPattern('/^172\.16\./');
	
	// Configure the Elasticsearch data source that will be shared by the next few signatures.
	$web_es = new ElasticsearchDataSource('http://logs.example.com:9200/', 'logstash');
	
	// Match POST requests that result in 403 responses.
	//
	// Real people can get these if they leave a form open over night and their session expires. Let them refresh
	// the page a few times before logging in. If they get more than a few of these, they are *very* likely malicious.
	...
	//
	// These are almost never submitted by real people and are a very good indication of malicious
	// behavior.
	$signature = new ElasticsearchSignature($web_es);
	...
	//
	// This is more common for valid clients, so allow clients to try a few submissions before
	// getting around to turning on Javascript.
	$signature = new ElasticsearchSignature($web_es);
